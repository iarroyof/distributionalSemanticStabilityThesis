...
#    data_t = array([[20.0,30.0,40.0],[10.0,20.0,30.0],[10.0,20.0,40.0]])
    ...
    param_grid = [ {'svm_c': expon(scale=100, loc=5),
                    ...
                    #'widths': [[expon(scale=100, loc=5), expon(scale=100, loc=5)]] 
                  }]
    params = param_grid[0]
    #for params in parameter_grid: 
    params['widths'] = []
    for s in xrange(16): # 16 weights maximun
        params['widths'].append(random.uniform(low=min_wigth, high=max_weight, size=s))

    mkl = mkl_regressor()
    rs = RS(mkl, param_distributions = params, n_iter = 10, n_jobs = 24, cv = k)#, scoring="r2", verbose=True)
    rs.fit(data, labels)
